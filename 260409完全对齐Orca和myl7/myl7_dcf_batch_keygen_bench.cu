...
using PrgType = fss::prg::ChaCha<4>;
    ...
    std::uint64_t n)
{
    ...
    dcf.Gen(cws + tid * static_cast<std::uint64_t>(kInBits + 1), s, alphas[tid], betas[tid]);
}
...
int main(int argc, char **argv)
{
    ...
    for (std::size_t offset = 0; offset < static_cast<std::size_t>(n); offset += chunkSize)
    {
        ...
        std::vector<int4> h_betas(chunk, dcf_bench::kPayload);
        for (std::size_t i = 0; i < chunk; ++i)
        {
            const auto idx = offset + i;
            h_seeds[i * 2] = dcf_bench::makeSeed(idx, 0x1111111111111111ULL);
            h_seeds[i * 2 + 1] = dcf_bench::makeSeed(idx, 0x2222222222222222ULL);
            h_alphas[i] = static_cast<InType>(20ULL + 2ULL * idx + 1ULL);
        }
        ...
        dcfGenKernel<<<numBlocks, kThreadsPerBlock>>>(
            d_cws,
            d_seeds,
            d_alphas,
            d_betas,
    ...
    std::printf(
        "myl7-gpu DCF batch-keygen benchmark finished\n"
        "  bin: %d bit\n"
        "  bout: 1 bit\n"
        "  n: %llu elem\n"
        "  keygen: %llu us\n",
