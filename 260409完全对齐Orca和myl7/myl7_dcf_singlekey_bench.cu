    ...
    const std::vector<int4> &cachedShare)
{
    for (std::size_t i = 0; i < xs.size(); ++i)
    {
        ...
        const std::uint64_t expected = static_cast<std::uint64_t>(xs[i] <= userAlpha);
        if (combined != expected)
        {
            std::fprintf(stderr,
                "Validation failed at i=%zu: alpha=%llu x=%llu expected=%llu got=%llu\n",
                i,
                static_cast<unsigned long long>(userAlpha),
...
int main(int argc, char **argv)
{
    ...
    const auto xs = dcf_bench::buildQueries(static_cast<std::size_t>(n), alpha);
    ...
    scheme.Gen(h_cws, seeds, keyAlpha, dcf_bench::kPayload);
    ...
    std::printf(
        "myl7-gpu DCF single-key benchmark finished\n"
        "  bin: %d bit\n"
        "  bout: %d bit\n"
        "  n: %llu elem\n"
        "  eval_iters: %d\n"
